...
class CanvasDrawer {
    ...
    /**
     * 
     * @param {{data: Uint32Array; length: number}} cells 
     */
    draw({ data, length }) {
        ...
        for (let ci = 0; ci < length; ci++) {
            const coord = data[di++];
            const x = (coord & 0xffff0000) >> 16;
            const y = coord & 0x0000ffff;
            const colour = data[di++] & 0x00ffffff;
    ...
    pixelToCellCoord(px, py) {
        return [
            Math.floor((px - this.#offset.x) * this.#cellSizeInv) & this.#wrapMask,
            Math.floor((py - this.#offset.y) * this.#cellSizeInv) & this.#wrapMask,
        ];
    }
    ...
    #calcVisibleCoordinates() {
        if (this.#canvas.width < this.#worldSize) {
            if (this.#offset.x <= 0) {
                this.#visibleCoordinates.x.start = Math.floor(-this.#offset.x * this.#cellSizeInv)
            } else {
                this.#visibleCoordinates.x.start = Math.floor(this.#axisLength - (this.#offset.x * this.#cellSizeInv))
            }
            this.#visibleCoordinates.x.end = this.#visibleCoordinates.x.start + Math.ceil(this.#canvas.width * this.#cellSizeInv);
        ...
        if (this.#canvas.height < this.#worldSize) {
            if (this.#offset.y <= 0) {
                this.#visibleCoordinates.y.start = Math.floor(-this.#offset.y * this.#cellSizeInv)
            } else {
                this.#visibleCoordinates.y.start = Math.floor(this.#axisLength - (this.#offset.y * this.#cellSizeInv))
            }
            this.#visibleCoordinates.y.end = this.#visibleCoordinates.y.start + Math.ceil(this.#canvas.height * this.#cellSizeInv);
        ...
        // console.log('VISIBLE WORLD', { ...this.#visibleCoordinates })
    }
